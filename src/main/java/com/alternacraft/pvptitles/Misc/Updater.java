...
public class Updater {
    ...
    public boolean shouldUpdate(String localVersion, String remoteVersion) {
        String[] localV = localVersion.split(Pattern.quote("."));
        String[] removeV = remoteVersion.split(Pattern.quote("."));

        int length = (localV.length <= removeV.length) ? localV.length : removeV.length;

        for (int i = 0; i < length; i++) {
            if (Integer.valueOf(localV[i]) < Integer.valueOf(removeV[i])) {
                return true;
            } else if (Integer.valueOf(localV[i]) > Integer.valueOf(removeV[i])) {
                return false;
            }
        }

        // Si sale quiere decir que, en la longitud que recorre el bucle, son iguales
        return removeV.length > localV.length;
    }
